...
While the acoustical emission of axial compressors has been a  subject of extensive research since more than fifty years ago \cite{nvemec1967noise,griffiths1964spectrum}, studies performed on centrifugal turbocharger are comparatively more scarce. 
...
Evans and Ward \cite{evans2005minimizing,evans2006reduction} opted for a simple setup of four microphones to measure sound radiation. These four microphones were situated on the same plane and directed to a turbocharger-equipped engine. They were distributed at 90º with a separation of one meter from the engine. From the pressure readings of these four microphones, Sound Pressure Level (SPL)\footnote{Usually defined as $10\log_{10}\left(\bar p^2/p_\text{ref}^2\right)$, with $\bar p$ being the root mean square (RMS) sound pressure and $p_\text{ref}=20\,\mu \text{Pa}$ being the threshold of human hearing.} was computed and then averaged among them, in order to get a single signal from which spectrograms and sound spectra could be derived.
...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.72\textwidth]{./figures/cap_liter/sample_sil}
